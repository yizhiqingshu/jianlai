...
long InBufArray0[NPT]={0};         //定义输入数组
long InBufArray1[NPT]={0};         //定义输入数组
...
//		  
        ...
        InBufArray0[fft_i]=ADC_ConvertedValue[0]<< 16;//对应PC1 的采集数据
        InBufArray1[fft_i]=ADC_ConvertedValue[1]<< 16;//对应PC0 的采集数据
        fft_i++;            
        }
//    for(i=0;i<NPT;i++)
//	InBufArray[i] = ((signed short)(ADC_ConvertedValue[i])) << 16;  //左移16位，高16位存放实部
...
//        cr4_fft_1024_stm32(OutBufArray, InBufArray0, NPT);
            InitBufInArray();
            cr4_fft_1024_stm32(OutBufArray, lBufInArray, NPT);
